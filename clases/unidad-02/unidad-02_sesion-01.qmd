...
## Ejercicio rápido: el suelo trifásico {.center}

Un suelo ideal en volumen es: 50% sólidos, 25% agua, 25% aire.
...
::: {.notes}
1. La fase líquida. 2. Disminuye la fase gaseosa y luego la líquida (baja la porosidad total). 3. Las raíces y microorganismos respiran; sin O₂ hay hipoxia.
:::
...
<!-- TODO: diapositiva desactivada — falta la imagen images/retencion_agua_arcillas.png.
Restaurar cuando se agregue la imagen:
...
## Ejercicio: área superficial y retención {.center}

1. ¿Por qué la arcilla retiene mucha más agua que la arena?
2. ¿Esa agua retenida por la arcilla está toda disponible para la planta? ¿Por qué?
...
::: {.notes}
1. Área superficial ~1,000–10,000× mayor. 2. No: parte queda adsorbida con Ψm < PMP. 3. Carga negativa; atrae cationes y dipolos de agua (doble capa difusa).
:::
...
## Ejercicio: textura, estructura y poros {.center}

1. ¿Qué tipo de poro retiene el agua DISPONIBLE para la planta?
2. Un suelo franco tiene mejor balance retención-disponibilidad que uno arcilloso. ¿Por qué?
3. ¿Puede un agricultor cambiar la textura de su suelo? ¿Y la estructura?
...
::: {.notes}
1. Mesoporos (0.2–30 μm). 2. El arcilloso retiene más agua total pero también más en PMP → ADT similar pero menos agua fácilmente aprovechable. 3. Textura no; estructura sí (MO, labranza, evitar compactación).
:::
...
## Curvas de retención por textura {.center}
...
#| fig-align: center
curvas <- tibble(
  psi = c(0, 5, 10, 33, 50, 100, 300, 500, 1000, 1500),
  Arenoso = c(0.42, 0.28, 0.18, 0.12, 0.09, 0.07, 0.05, 0.04, 0.035, 0.03),
  Franco = c(0.48, 0.42, 0.38, 0.30, 0.26, 0.22, 0.17, 0.14, 0.12, 0.11),
  Arcilloso = c(0.55, 0.52, 0.49, 0.40, 0.36, 0.32, 0.26, 0.24, 0.22, 0.21)
) |>
  pivot_longer(-psi, names_to = "suelo", values_to = "theta") |>
  mutate(psi_plot = ifelse(psi == 0, 1, psi),
         suelo = factor(suelo, levels = c("Arenoso", "Franco", "Arcilloso")))

etiquetas <- curvas |> filter(psi == 1500)
f1 <- ggplot(curvas, aes(psi_plot, theta, colour = suelo, shape = suelo)) +
  annotate("rect", xmin = 33, xmax = 1500, ymin = -Inf, ymax = Inf, fill = col_agua[1], alpha = 0.6) +
  geom_vline(xintercept = c(33, 1500), linetype = "dashed", colour = col_ref) +
  annotate("text", x = 33, y = 0.585, label = "CC\n−33 kPa", colour = col_texto2, size = 4.5, vjust = 1, hjust = -0.1) +
  annotate("text", x = 1500, y = 0.585, label = "PMP\n−1500 kPa", colour = col_texto2, size = 4.5, vjust = 1, hjust = 1.1) +
  annotate("text", x = 220, y = 0.585, label = "Agua disponible (ADT)", colour = col_texto2, size = 4.5, vjust = 1) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  geom_text(data = etiquetas, aes(label = suelo), hjust = -0.2, size = 5, show.legend = FALSE) +
  scale_colour_manual(values = col_serie) +
  scale_shape_manual(values = c(16, 15, 17)) +
  scale_x_log10(breaks = c(1, 10, 33, 100, 500, 1500), labels = c("0 (Sat)", "10", "33", "100", "500", "1500"),
                expand = expansion(mult = c(0.03, 0.22))) +
  scale_y_continuous(limits = c(0, 0.59), breaks = seq(0, 0.5, 0.1)) +
  labs(x = "Potencial mátrico |Ψm| (kPa, escala log)", y = "θv (cm³/cm³)") +
  theme(legend.position = "none")
f1
```

::: {style="font-size: 0.8em; text-align: center;"}
La franja celeste es el agua disponible (CC → PMP): la arena la pierde casi toda antes de −33 kPa.
:::
...
## Ejercicio: lee la curva de retención {.center}

Para un suelo franco: θ a −33 kPa = 0.30 y θ a −1500 kPa = 0.11.

1. ¿Cuánta agua disponible (ADT) hay?
2. Si la θv actual es 0.20, ¿el suelo está húmedo o seco respecto a CC?
3. ¿Qué ocurre con θ cuando Ψm se hace más negativo?
...
::: {.notes}
1. ADT = 0.30 − 0.11 = 0.19. 2. Seco (0.20 < CC = 0.30; se han consumido ~0.10). 3. θ disminuye (menos agua a mayor succión).
:::
...
::: {.notes}
1. Agua a CC: B (0.38) > A (0.20). ADT: A = 0.20 − 0.07 = 0.13; B = 0.38 − 0.19 = 0.19 → B tiene más agua total y más ADT. 2. Más arcilla protege la MO de la descomposición (agregados, complejos arcillo-húmicos) y permite más biomasa; también puede influir el manejo (menos laboreo). La MO a su vez baja la Da (1.15 vs 1.50). 3. AFA lechuga: A = 0.13 × 0.30 = 0.039; B = 0.19 × 0.30 = 0.057 → en B se riega con menor frecuencia (~46% más de reserva).
:::
